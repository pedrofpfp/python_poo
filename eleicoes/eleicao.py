...
class Urna:
    ...
    __secao : int
    __zona : int
    __eleitores_presentes : List[Eleitor] = [0]
    ...
    def registrar_voto(self, eleitor : Eleitor, n_cand : int):
        ...
        with open(self.__nome_arquivo, 'wb') as arquivo:
            pickle.dump(self.__votos, arquivo)
    def emitirZeresima(self):



        with open(f'zeresima_'+self.__nome_arquivo, 'wb') as arquivoZeresima:
            pickle.dump(self.__votos, arquivoZeresima)
        return self.__str__()
